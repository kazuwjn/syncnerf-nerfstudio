...
from syncnerf.syncnerf import KPlanesModelConfig
from syncnerf.syncnerf_dataparser import SyncNeRFDataParserConfig
from syncnerf.fourdv_dataparser import FourDVDataParserConfig


syncnerf_kplanes = MethodSpecification(
    config=TrainerConfig(
        method_name="syncnerf-kplanes",
        ...
        pipeline=VanillaPipelineConfig(
            datamanager=VanillaDataManagerConfig(
                dataparser=SyncNeRFDataParserConfig(),
    ...
    description="Sync-NeRF K-Planes model"
)


syncnerf_kplanes_4dv = MethodSpecification(
    config=TrainerConfig(
        method_name="syncnerf-kplanes-4dv",
        steps_per_eval_batch=500,
        steps_per_save=2000,
        steps_per_eval_all_images=30000,
        max_num_iterations=30001,
        ...
        pipeline=VanillaPipelineConfig(
            datamanager=VanillaDataManagerConfig(
                dataparser=FourDVDataParserConfig(),
                train_num_rays_per_batch=4096,
                eval_num_rays_per_batch=4096,
            ),
            model=KPlanesModelConfig(
    ...
    description="Sync-NeRF K-Planes model (4dv)"
)