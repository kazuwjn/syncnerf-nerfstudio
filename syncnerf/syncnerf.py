...
from syncnerf.syncnerf_field import KPlanesDensityField, KPlanesField
...
class KPlanesModel(Model):
    ...
    def populate_modules(self):
        ...
        self.num_cameras = self.kwargs["metadata"]["num_cameras"]
        self.num_frames = self.kwargs["metadata"]["num_frames"]
        self.camera_ids = self.kwargs["metadata"]["camera_ids"]
        self.camera_ids = torch.tensor(self.camera_ids, device=0)

        # Init camera offset
        self.camera_offset = torch.nn.Parameter(torch.zeros(self.num_cameras))
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        # offset
        if self.config.use_camera_offset:
            ray_bundle.times = ray_bundle.times + self.camera_offset[self.camera_ids[ray_bundle.camera_indices]]
    ...
    def get_image_metrics_and_images(
        ...
        # TODO: ValueError: Expected both input arguments to be normalized tensors with shape [N, 3, H, W]. ...
